...
int main(int argc, const char **argv) {
    ...
    string output_file_name = "output.sas";

    for (int i = 1; i < argc; ++i) {
        string arg = string(argv[i]);
        if (arg.compare("--no_rel") == 0) {
            std::cout << "*** do not perform relevance analysis ***" << endl;
            g_do_not_prune_variables = true;
        } else if (arg.compare("--h2_output_file") == 0) {
            i++;
            if (i < argc) {
                try {
                    output_file_name = string(argv[i]);
                }catch (std::invalid_argument) {
                    cerr << "please specify the file name after --h2_output_file" << endl;
                    exit(2);
                }
            } else {
                cerr << "please specify the file name after --h2_output_file" << endl;
            ...
            std::cout << "Usage: ./preprocess [--no_rel] [--no_h2]  [--no_bw_h2] [--augmented_pre] [--stat] < output" << endl;
            exit(2);
        }
    }
    ...
    std::cout << "Building causal graph..." << endl;
    ...
    // compute h2 mutexes
    if (axioms.size() > 0) {
        std::cout << "Disabling h2 analysis because it does not currently support axioms" << endl;
        ...
        if(!compute_h2_mutexes(ordering, operators, axioms,
            ...
            std::cout << "Unsolvable task in preprocessor" << endl;
            generate_unsolvable_cpp_input(output_file_name);
            std::cout << "done" << endl;
            return 0;
	}
        ...
        std::cout << "Change id of operators: " << operators.size() << endl;
        ...
        // std::cout << "Change id of axioms: " << axioms.size() << endl;
        // for(int i = 0; i < axioms.size(); ++i){
        //     axioms[i].remove_unreachable_facts();
        // }
        std::cout << "Change id of mutexes" << endl;
        for (MutexGroup &mutex : mutexes) {
            mutex.remove_unreachable_facts();
        }
        std::cout << "Change id of goals" << endl;
        ...
        std::cout << "Change id of initial state" << endl;
        ...
        std::cout << "Remove unreachable facts from variables: " << ordering.size() << endl;
    ...
    std::cout << "Building domain transition graphs..." << endl;
    ...
    std::cout << "solveable in poly time " << solveable_in_poly_time << endl;
    std::cout << "Building successor generator..." << endl;
    ...
    std::cout << "Preprocessor variables: " << ordering.size() << endl;
    std::cout << "Preprocessor facts: " << facts << endl;
    std::cout << "Preprocessor derived variables: " << derived_vars << endl;
    std::cout << "Preprocessor operators: " << operators.size() << endl;
    std::cout << "Preprocessor mutex groups: " << mutexes.size() << endl;
    ...
    if (expensive_statistics) {
        ...
        std::cout << "Augmented preconditions: " << num_total_augmented << endl;
        std::cout << "Ops with augmented preconditions: " << num_op_augmented << endl;
        std::cout << "Potential preconditions: " << num_total_potential << endl;
        std::cout << "Ops with potential preconditions: " << num_op_potential << endl;
        std::cout << "Potential preconditions contradict effects: " << num_total_potential_noeff << endl;
        std::cout << "Ops with potential preconditions contradict effects: " << num_op_potential_noeff << endl;
        ...
        std::cout << "Preprocessor mutex groups fw: " << mutexes_fw.size()
    ...
    std::cout << "Preprocessor task size: " << task_size << endl;
    ...
    if (ordering.empty()) {
        ...
        generate_cpp_input(output_file_name,
