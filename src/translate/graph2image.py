...
def create_raw_matrix_for_image(graph, bolded=False, shrink_ratio=6):
    ...
    ## Creating a matrix
    logging.debug("Creating matrix for a graph with %s nodes.." % sz)
    ...
    if sz > MAX_SIZE_EXPLICIT:
        ...
        logging.debug("Matrix size when created: %s" % (matrix_data.data.nbytes + matrix_data.rows.nbytes))
    else:
        matrix_data = np.zeros((sz,sz), dtype=int)
        logging.debug("Matrix size when created: %s" % matrix_data.nbytes)

    logging.debug("Matrix created, filling with values for edges..")
    if bolded:
        logging.debug("Performing bolding.")
    ...
    logging.debug("Matrix size when 1s added: %s" % nbytes_size)
...
def print_graph_statistics(graph):
    ...
    logging.debug("Number of graph vertices: %s" % sz)
    logging.debug("Number of graph edges: %s" % matrix_data.count_nonzero())
...
def shrink_matrix_raw_to_grayscale(graph, bolded=False, shrink_ratio=6):
    ...
    logging.debug("Number of graph nodes: %s" % sz)
    logging.debug("Shrink ratio: %s" % shrink_ratio)
    if shrink_ratio == 1:
        return matrix_data

    shrinked_sz = int(math.ceil(float(sz)/shrink_ratio))
    n = 0
    logging.debug("Shrinking matrix to size %sx%s.." % (shrinked_sz,shrinked_sz))

    if shrinked_sz > MAX_SIZE_EXPLICIT:
        shrinked_matrix_data_test = lil_matrix((shrinked_sz, shrinked_sz), dtype=int)
        logging.debug("Shrinked matrix size when created: %s" % (shrinked_matrix_data_test.data.nbytes + shrinked_matrix_data_test.rows.nbytes))
    else:
        shrinked_matrix_data_test = np.zeros((shrinked_sz,shrinked_sz), dtype=int)
        logging.debug("Shrinked matrix size when created: %s" % shrinked_matrix_data_test.nbytes)
...
def write_matrix_image_grayscale(graph, output_directory, bolded=False, shrink_ratio=6, target_size=128, write_original_size=False):
    ...
    logging.debug("Grayscale color: %s" % grayscale_color)
    ...
    #logging.debug(matrix_data[matrix_data.nonzero()])
    ...
    for x,y,color in zip(cx.row, cx.col, cx.data):
        # logging.info("Pixel of color %s at (%s,%s)" % (grayscale_color - color, x, y))
    ...
    if write_original_size:
        logging.debug("Writing grayscale image of size %sx%s .." % (sz, sz))
...
if __name__ == "__main__":
    ...
    if not os.path.exists(image_output_directory):
        ...
        logging.info("Using image output directory {}".format(image_output_directory))
    ...
    logging.info("Done computing image! %s" % timer)
